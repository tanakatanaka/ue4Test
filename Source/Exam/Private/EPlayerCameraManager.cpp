...
AEPlayerCameraManager::AEPlayerCameraManager(const class FObjectInitializer& ObjectInitializer)
...
void AEPlayerCameraManager::BeginPlay()
{
	Super::BeginPlay();
...
void AEPlayerCameraManager::UpdateCamera(float DeltaTime)
{
	AExamCharacter* MyPawn = PCOwner ? Cast<AExamCharacter>(PCOwner->GetPawn()) : nullptr;
	if (MyPawn)
	{
